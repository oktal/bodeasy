...
inline void toStatusMessage(const QString& string)
{
	qDebug() << string.toLocal8Bit().constData();
}

inline bool toIsIdent(const QChar& c)
{
...
//              qDebug() << "first != (\"END\") ";
			 ...
			 do
			 {
				 ...
				 {
					 toStatusMessage("toSQLparse: Unbalanced parenthesis (Too many ')')");
				 }
...
QString toSQLParse::indentStatement(statement &stat, int level/*SQLITEMAN, toSyntaxAnalyzer &syntax*/)
{
	...
	default:
		{
			toStatusMessage("toSQLparse: Internal error in toSQLParse, should never get here");
		}
